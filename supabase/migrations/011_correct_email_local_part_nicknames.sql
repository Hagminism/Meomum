-- Some identity providers expose the email local part as `nickname`.
-- Treat that generated value like the full email when a user next signs in.
...
BEGIN
  ...
  v_profile_name := COALESCE(
    ...
    CASE
      WHEN v_name IS NOT NULL
        ...
        AND LOWER(v_name) IS DISTINCT FROM LOWER(SPLIT_PART(v_email, '@', 1))
        THEN v_name
    END,
  ...
  ON CONFLICT (account_id) DO UPDATE
    SET nickname = EXCLUDED.nickname,
        ...
        updated_at = now()
    -- Correct generated placeholder or email-derived nicknames only. A
    -- user-selected nickname must never be overwritten during a later login.
    WHERE public.profiles.nickname = ''
      OR public.profiles.nickname = '사용자'
      OR (
        v_email IS NOT NULL
        AND (
          LOWER(public.profiles.nickname) = LOWER(v_email)
          OR LOWER(public.profiles.nickname) = LOWER(SPLIT_PART(v_email, '@', 1))
        )
      );
